...
-- Madicinal +Mod 1.0
-- UI-only layer for the CoA Ascension fork. The upstream AuctionatorBuy.lua
-- still owns querying, exact live-row validation, PlaceAuctionBid, server ACK,
-- stale-row handling, retries and displayed-scan updates.
...
local function RefreshRows()
    ...
    if offset + rowsPerPage < #currentMatches then
        frame.next:Enable();
    else
        frame.next:Disable();
    end
    ...
    if #currentMatches > 0 then
        frame.page:SetText(string.format(
            "Showing %d-%d of %d live matching auctions",
            offset + 1,
            math.min(#currentMatches, offset + rowsPerPage),
            #currentMatches
        ));
    else
...
local function CreatePicker()
    ...
    for i = 1, rowsPerPage do
        ...
        rf.num:SetPoint("LEFT", 4, 0);
        rf.num:SetWidth(40);
        rf.num:SetJustifyH("LEFT");

        rf.item = rf:CreateFontString(nil, "OVERLAY", "GameFontHighlightSmall");
        rf.item:SetPoint("LEFT", 50, 0);
        rf.item:SetWidth(178);
        rf.item:SetJustifyH("LEFT");

        rf.owner = rf:CreateFontString(nil, "OVERLAY", "GameFontHighlightSmall");
        rf.owner:SetPoint("LEFT", 234, 0);
        rf.owner:SetWidth(132);
        rf.owner:SetJustifyH("LEFT");

        rf.price = rf:CreateFontString(nil, "OVERLAY", "GameFontHighlightSmall");
        rf.price:SetPoint("LEFT", 372, 0);
        rf.price:SetWidth(120);
        rf.price:SetJustifyH("LEFT");

        rf.index = rf:CreateFontString(nil, "OVERLAY", "GameFontDisableSmall");
        rf.index:SetPoint("BOTTOMLEFT", 50, 1);
        rf.index:SetWidth(178);
        rf.index:SetJustifyH("LEFT");

        rf.buy = CreateFrame("Button", nil, rf, "UIPanelButtonTemplate");
        rf.buy:SetWidth(74);
        rf.buy:SetHeight(21);
        rf.buy:SetPoint("RIGHT", -4, 0);
        rf.buy:SetText("Buy");
    ...
    frame.close:SetText("Done");
    frame.close:SetScript("OnClick", function()
        if Atr_Buy_Cancel then
...
-- Tag the inherited confirmation path so Ascension's blocked-action event tells
-- us whether the native Buy/Buy Next path or our row picker reached PlaceAuctionBid.
...
blockedWatch:SetScript("OnEvent", function(self, eventName, addonName, functionName)
...
controller:SetScript("OnUpdate", function(self, elapsed)
    ...
    local button = Atr_BuyMultiple_Button;
    if not button or not Atr_Buy1_Button then return; end

    -- This XML-declared button is our load indicator as well as the feature entry.
    -- It stays visible whenever native Buy is visible, even when disabled.
    if Atr_Buy1_Button:IsShown() and (not AuctionFrame or AuctionFrame:IsShown()) then
        button:Show();
        button:SetFrameLevel(math.max((Atr_Buy1_Button:GetFrameLevel() or 1) + 8, 20));
    else
        button:Hide();
        return;
    end
