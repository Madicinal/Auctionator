...
-- Return an item string for the same item as it would be at a given character
-- level.  On Ascension one item id is auctioned at many levels and every auction
-- hands back the same base link, so SetHyperlink on that link always renders one
-- variant (usually the seller's own).  Field LEVEL is what the client scales the
-- stats from, so overriding it is the only way to show the tooltip of the exact
-- auction under the cursor.  Harmless for unscaled items, which ignore the field.

function Atr_ItemLinkAtLevel(itemLink, level)
  if type(itemLink) ~= "string" or type(level) ~= "number" or level <= 0 then
    return itemLink
  end
  ...
  local levelField = Auctionator.Constants.ItemLink.LEVEL
  local fields = {}
  local index

  for index = 1, math.max(#parsed, levelField) do
    fields[index] = Atr_NormalizeVariantField(parsed[index])
  end

  fields[levelField] = tostring(level)
